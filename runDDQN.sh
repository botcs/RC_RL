...
for game in ${games[*]}; do
    ...
    # send the job to NCF
    #
    sbatch_output=`sbatch -p fasse_gpu --gres=gpu --mem 20001 -t 2-0:20 -o ${outfileprefix}_%j.out -e ${outfileprefix}_%j.err --wrap="source activate pedro; python -W ignore::UserWarning runDDQN.py -timeout=1200 -max_steps=1000000 -max_level_steps=1200 -level_switch=fmri -game_name=${game}"`
    # for local testing
